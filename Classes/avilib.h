...
#define  xio_open		_open

#define  xio_read			_read

#define  xio_write		_write

//int xio_ftruncate(int fd, off_t length)
//{
//	return ftruncate
//}
#define  xio_lseek		_lseek

#define  xio_close		_close

#define xio_fstat			_fstat

//int xio_lstat(const char *filename, struct stat *buf)
//{
//	return 
//}
#define  xio_stat			_stat
...
typedef struct
{
	...
	FILE *pfile;			//add by chlx

	  long   mode;              /* 0 for reading, 1 for writing */
	  
	  //add by chlx 增加I/O缓存区, 暂时实现写操作
	 // long   buf_size;
	 // long   buf_pos;
	  //char   *pbuf;
...
avi_t* AVI_open_output_file(char * filename, long iobufsize = 4096);
...
avi_t *AVI_open_fd(FILE *pf, int getIndex);
avi_t *AVI_open_indexfd(FILE*pf, int getIndex, char *indexfile);
