...
public:
    ...
    void SetRunCarDelegate(id delegate);
    void SetRunCarRemoteDownloadDelegate(id delegate);
    int SetSDCardRecord(char * user ,char *pwd,int status);
public:    
...
private:    
    ...
    void PPPPClose();
    void ImageNotify(UIImage *image, unsigned int timestamp,unsigned int streamid);
    void YUVNotify(unsigned char* yuv, int len, int width, int height, unsigned int timestamp,unsigned int streamid);
    ...
    void ProcessRunCarModeResult(STRU_CAMERA_STATUS cameraStatus);
    void ProcessRemoteFilesResult(NSString *fileName,int filesize,int bEnd);
    void PlaybackDownloadDataResult(char *pbuf,unsigned int length ,unsigned int currentData,unsigned int totalData);
...
private:        
    ...
    NSCondition *m_RunCarLock;
    id<RunCarModeProtocol> m_RunCarDelegate;
    
    NSCondition *m_RunCarDownloadRemoteFileLock;
    id<RunCarDownloadRemoteFileProtocol>m_RunCarRemoteDownloadDelegate;
